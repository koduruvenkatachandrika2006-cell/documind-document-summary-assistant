...
export function App() {
  ...
  const processFileFlow = async (file: File) => {
    ...
    try {
      ...
      console.error('[App] Document processing flow failed:', error);
      setErrorMessage(error.message || 'Unable to process this document. Please try again.');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleLoadSample = async (sampleType: 'proposal' | 'receipt') => {
    ...
    try {
      ...
      await processFileFlow(file);
    } catch (err: any) {
      console.error(`[App] Sample ${sampleType} processing failed:`, err);
      setErrorMessage(err.message || 'Unable to process sample document. Please try again.');
      setIsProcessing(false);
    }
  };
...
`;
  ...
  const isImageFile = Boolean(
    selectedFile && (
      ...
      ['png', 'jpg', 'jpeg', 'webp'].includes(selectedFile.name.split('.').pop()?.toLowerCase() || '')
    )
  );
